...
def display_dashboard_content(
    ...
    if 'parent_url' in df_display.columns:
        ...
        if len(unique_posts) > 0: # Só mostra o filtro se houver posts
            ...
            if post_options_map:
                ...
                final_post_options = ["All Posts"] + sorted(list(post_options_map.keys()))
                
                selected_post_display = st.sidebar.selectbox(
                    f"Post Específico ({network_name}):", 
                    final_post_options, 
                    key=f"post_filter_{profile_name}_{network_name}",
                    help="Selecione um post específico para analisar os comentários."
                )
    ...
    # --- <<< INÍCIO DA MUDANÇA: APLICAÇÃO DOS FILTROS (EM CADEIA) >>> ---
    active_filters_list = []
    
    # 1. Aplica o filtro de POST (NOVO)
    ...
    # --- <<< FIM DA MUDANÇA: APLICAÇÃO DOS FILTROS >>> ---
