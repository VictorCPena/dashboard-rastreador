# dashboard.py (CORRIGIDO: Separa Posts de Comentários + Adiciona Aba de Análise por Post)
# -*- coding: utf-8 -*-
...
def _preprocess_dataframe(df_full: pd.DataFrame) -> pd.DataFrame:
    ...
    # Adiciona 'parent_url' (o link de ouro) aos defaults
    ...
    # parent_url é OK ter nulos (são os posts-mãe)
...
# --- ALTERAÇÃO: Título mudou para focar em Comentários ---
def plot_metrics(df_period_B, df_period_A):
    st.subheader(f"Métricas Gerais (Baseado em Comentários do Público)")
    if df_period_B.empty and df_period_A.empty: st.warning("Sem comentários do público nos períodos."); return
    ...
    # --- ALTERAÇÃO: Título do card ---
    col1.metric("Total de Comentários", b_total, delta=delta_strs["total"], delta_color="off")
    ...
    st.caption("Variação (Δ) vs. Período Comparação. (Não inclui legendas de posts)")
...
# --- <<< ALTERAÇÃO: Lógica de IA agora recebe DF já filtrado (só comentários) >>> ---
@st.cache_data(ttl=900)
def _run_gemini_logic_on_dataframe(df_filtrado_comentarios: pd.DataFrame) -> str:
    print("Iniciando _run_gemini_logic_on_dataframe...")
    st.session_state['last_ai_log_stderr'] = "" 
    try:
        gemini_key = st.secrets["GEMINI_API_KEY"]
        if not gemini_key:
            logging.error("ERRO_API_CONFIG: 'GEMINI_API_KEY' não encontrada.")
            return "ERRO_API_CONFIG: 'GEMINI_API_KEY' não encontrada."
    ...
    if df_filtrado_comentarios.empty:
        logging.warning("DataFrame (comentários) vazio fornecido para IA.")
        return "Não há comentários do público no período selecionado para gerar o resumo."
    
    df_analise = df_filtrado_comentarios.copy()
...
def run_ai_summary_generation(df_filtrado_comentarios: pd.DataFrame):
    st.info("Executando resumo IA (Gemini) sobre os comentários do público...")
    st.session_state['last_ai_log_stderr'] = ""
    st.session_state['last_ai_log_stdout'] = ""

    if df_filtrado_comentarios.empty:
        st.warning("Não há comentários do público no período selecionado para o resumo IA.")
        st.session_state['last_ai_log_stderr'] = "DataFrame (comentários) vazio fornecido."
        return None
    
    try:
        summary = _run_gemini_logic_on_dataframe(df_filtrado_comentarios)
...
# --- <<< INÍCIO DA CORREÇÃO: HTML (Baseado em Comentários) >>> ---
def generate_html_report(df_to_save: pd.DataFrame, summary_text: str, profile_name_for_file: str,
    ...
    # --- ALTERAÇÃO: Filtra o DF para usar APENAS comentários ---
    # Se 'parent_url' NÃO é Nulo (notnull), é um comentário
    df_comentarios = df_to_save[df_to_save['parent_url'].notnull()].copy()
    
    if df_comentarios.empty:
        st.warning("Nenhum comentário do público encontrado para o relatório HTML.")
    
    # --- 1. Métricas e Pizza de Sentimento ---
    try:
        counts_b = df_comentarios['sentimento'].value_counts()
        b_total = len(df_comentarios)
    ...
    # --- 2. Pizza de Gênero ---
    try:
        gender_counts = df_comentarios['genero_previsto'].value_counts()
    ...
    # --- 3. Barras Gênero x Sentimento ---
    try:
        gender_sentiment = df_comentarios.groupby(['genero_previsto', 'sentimento']).size().unstack(fill_value=0)
        if not gender_sentiment.empty:
            datasets = []
            for sent in gender_sentiment.columns:
                datasets.append({
                    "label": str(sent), 
                    "data": [int(v) for v in gender_sentiment[sent].values]
                })
    ...
    try:
        palavras = get_cleaned_words_for_freq(tuple(df_comentarios["texto_puro"].astype(str).tolist()))
        if palavras: 
    ...
    # --- 5. Frequência de Emojis ---
    try:
        df_comentarios['emojis'] = df_comentarios['emojis'].apply(lambda x: x if isinstance(x, list) else [])
        all_emojis = [e for sublist in df_comentarios['emojis'] for e in sublist]
        if all_emojis: 
    ...
    # --- 7. Gráfico de Tamanho Médio ---
    try:
        if 'tamanho_comentario' in df_comentarios.columns and 'sentimento' in df_comentarios.columns:
            avg_length = df_comentarios.groupby('sentimento')['tamanho_comentario'].mean().round(1)
    ...
    # --- Geração da Tabela (Amostra de Comentários) ---
    try:
        cols = ['usuario', 'conteudo', 'sentimento', 'genero_previsto', 'data_hora', 'fonte_coleta']
        ex_cols = [c for c in cols if c in df_comentarios.columns]
        df_d = df_comentarios.sort_values(by='data_hora', ascending=False)[ex_cols].head(10).copy()
...
# --- ALTERAÇÃO: Lógica de divisão (Posts vs Comentários) movida para cá ---
def display_dashboard_content(
    ...
    df_filtered_A_full, df_filtered_B_full = pd.DataFrame(), pd.DataFrame()
    ...
    if len(date_range_A) == 2:
        ...
        if start_A and end_A:
            ...
            try:
                df_filtered_A_full = _df_final_filtered[(_df_final_filtered['data_hora'].dt.date >= start_A) & (_df_final_filtered['data_hora'].dt.date <= end_A)].copy()
                valid_A = True
    ...
    if len(date_range_B) == 2:
        ...
        if start_B and end_B:
            ...
            try:
                df_filtered_B_full = _df_final_filtered[(_df_final_filtered['data_hora'].dt.date >= start_B) & (_df_final_filtered['data_hora'].dt.date <= end_B)].copy()
                valid_B = True
            except Exception as e: st.error(f"Erro filtro B: {e}")

    # --- INÍCIO DA LÓGICA DE SEPARAÇÃO (A MÁGICA ACONTECE AQUI) ---
    
    # 1. Separa os Posts (Legendas) dos Comentários para o Período B
    df_posts_B = pd.DataFrame()
    df_comentarios_B = pd.DataFrame()
    if valid_B and not df_filtered_B_full.empty:
        df_posts_B = df_filtered_B_full[df_filtered_B_full['parent_url'].isnull()]
        df_comentarios_B = df_filtered_B_full[df_filtered_B_full['parent_url'].notnull()]
        
    # 2. Separa os Posts (Legendas) dos Comentários para o Período A
    df_posts_A = pd.DataFrame()
    df_comentarios_A = pd.DataFrame()
    if valid_A and not df_filtered_A_full.empty:
        df_posts_A = df_filtered_A_full[df_filtered_A_full['parent_url'].isnull()]
        df_comentarios_A = df_filtered_A_full[df_filtered_A_full['parent_url'].notnull()]
    ...
    if _df_final_filtered.empty and active_filters_list:
        ...
        # --- ALTERAÇÃO: Todos os gráficos usam df_comentarios_B ---
        
        critical_msg = check_critical_situation(df_comentarios_B, threshold_neg_percent=CRITICAL_NEG_THRESHOLD)
        if critical_msg: st.markdown(f'<div class="critical-alert">{critical_msg}</div>', unsafe_allow_html=True)

        # Passa os DFs de *comentários* para as métricas
        plot_metrics(df_comentarios_B, df_comentarios_A)

        st.header(f"Análise Detalhada (Período Principal{filter_title_string})")
        st.caption("Atenção: Todos os gráficos abaixo analisam APENAS os comentários do público (legendas de posts foram excluídas).")

        # --- ALTERAÇÃO: Adiciona nova aba "Análise por Post" ---
        ...
        with sub_tab1:
             if not df_comentarios_B.empty:
                 col1, col2 = st.columns(2)
                 with col1: st.plotly_chart(get_fig_pie_chart(df_comentarios_B), use_container_width=True)
                 with col2: st.plotly_chart(get_fig_gender_chart(df_comentarios_B), use_container_width=True)
             else: st.info(f"Sem comentários do público para os filtros.")
        with sub_tab2:
            if not df_comentarios_B.empty:
                col3, col4 = st.columns(2)
                with col3:
                    with st.spinner("Analisando freq..."):
                        st.plotly_chart(get_word_frequency_fig(df_comentarios_B), use_container_width=True) 
                with col4: st.plotly_chart(get_fig_top_emojis(df_comentarios_B), use_container_width=True)
                st.plotly_chart(get_fig_comment_length(df_comentarios_B), use_container_width=True)
            else: st.info(f"Sem comentários do público para os filtros.")
        with sub_tab3:
            if not df_comentarios_B.empty:
                st.plotly_chart(get_fig_timeline(df_comentarios_B), use_container_width=True)
                st.subheader(f"Amostra de Comentários (Período Principal{filter_title_string})")
                cols_s = ['usuario', 'conteudo', 'sentimento', 'genero_previsto', 'data_hora', 'fonte_coleta', 'run_id']; ex_s = [c for c in cols_s if c in df_comentarios_B.columns]
                df_ds = df_comentarios_B[ex_s].head(20).copy()
        ...
        # --- ESSA É A NOVA ABA QUE VOCÊ QUERIA ---
        with sub_tab4:
            st.subheader("Filtrar Comentários por Post Específico")
            
            # Usa df_posts_B (só as legendas)
            if df_posts_B.empty:
                st.warning("Nenhum post original (legenda) encontrado neste período/filtro.")
            else:
                # Cria uma "opção" amigável para o SelectBox
                df_posts_B_sorted = df_posts_B.sort_values(by='data_hora', ascending=False)
                df_posts_B_sorted['display_legenda'] = (
                    df_posts_B_sorted['data_hora'].dt.strftime('%d/%m') + " - " + 
                    df_posts_B_sorted['conteudo'].str.slice(0, 70) + "..."
                )
                
                # Pega o 'post_url' do post que o usuário selecionar
                selected_display = st.selectbox(
                    "Selecione um post (pela legenda) para ver seus comentários:",
                    df_posts_B_sorted['display_legenda']
                )
                
                # Encontra o 'post_url' real da legenda selecionada
                selected_post_url = df_posts_B_sorted[
                    df_posts_B_sorted['display_legenda'] == selected_display
                ].iloc[0]['post_url']
                
                # Mostra a legenda completa
                legenda_completa = df_posts_B_sorted[
                    df_posts_B_sorted['display_legenda'] == selected_display
                ].iloc[0]['conteudo']
                
                with st.expander("Ver legenda completa do post"):
                    st.markdown(legenda_completa.replace('\n', '<br>'), unsafe_allow_html=True)
                st.markdown(f"**Post URL:** `{selected_post_url}`")
                
                # Filtra os comentários REAIS que têm esse 'post_url' como 'parent_url'
                comentarios_do_post = df_comentarios_B[
                    df_comentarios_B['parent_url'] == selected_post_url
                ]
                
                st.markdown(f"---")
                st.markdown(f"**{len(comentarios_do_post)} comentários encontrados para este post:**")
                
                if not comentarios_do_post.empty:
                    # Mostra as estatísticas SÓ DESSE POST
                    col_post_1, col_post_2 = st.columns(2)
                    with col_post_1:
                        st.plotly_chart(get_fig_pie_chart(comentarios_do_post), use_container_width=True)
                    with col_post_2:
                        st.plotly_chart(get_fig_gender_chart(comentarios_do_post), use_container_width=True)
                        
                    st.dataframe(comentarios_do_post[['usuario', 'conteudo', 'sentimento', 'genero_previsto', 'data_hora']], use_container_width=True)
                else:
                    st.info("Nenhum comentário de público encontrado para este post específico (apenas a legenda).")
    ...
    if valid_B and valid_B_global: 
        ...
        if st.sidebar.button("Gerar Relatório HTML (Visão Atual)", key=f"btn_gen_{profile_name}_{network_name}", use_container_width=True, help=html_button_help):
            ...
            # --- ALTERAÇÃO: Passa o DF *completo* (com posts e comentários) ---
            # A função generate_html_report agora sabe filtrar
            df_to_export = df_filtered_B_full 
...
if selected_profile_name != "--- Selecione um Perfil ---":
    ...
    if not _df_full.empty and 'data_hora' in _df_full.columns and not _df_full['data_hora'].isnull().all():
        ...
        if valid_B_global:
            ...
            if st.sidebar.button("Gerar Resumo IA (Período Principal)", key=f"btn_ai_{profile_name}", use_container_width=True, help=f"Analisa {start_B_ai.strftime('%d/%m')} a {end_B_ai.strftime('%d/%m')} (TODAS as redes)."):
                ...
                try:
                    ...
                    # --- ALTERAÇÃO: Filtra para passar SÓ comentários para a IA ---
                    df_comentarios_para_ia = df_para_ia[df_para_ia['parent_url'].notnull()]
                    
                    print(f"Filtrados {len(df_comentarios_para_ia)} comentários para IA.")
                except Exception as e:
                    st.error(f"Erro ao filtrar dados para IA: {e}")
                    df_comentarios_para_ia = pd.DataFrame() 

                with st.spinner("Gerando Resumo IA (Gemini)..."):
                    summary_result = run_ai_summary_generation(df_comentarios_para_ia) 
        ...
        if st.session_state.get('summary_period_key') == summary_key_current and st.session_state.get('generated_summary'):
            ...
            st.caption("Nota: Resumo IA analisa **apenas os comentários do público** (não as legendas) de todas as redes do período.")
